...
vd_log_init "components" "$SCRIPT_NAME" "$LOG_DIR/full-stack.jsonl"
...
build_web_start_command() {
  ...
  if [[ -x "$next_bin" ]]; then
    printf '%s\0' "bash" "-lc" "cd \"$WEB_RUNTIME_WEB_DIR\" && export VIDEO_ANALYSIS_REPO_ROOT=\"$ROOT_DIR\" && exec ./node_modules/.bin/next dev --hostname 127.0.0.1 --port \"$WEB_PORT\""
    return 0
  fi

  printf '%s\0' "env" "VIDEO_ANALYSIS_REPO_ROOT=$ROOT_DIR" "npm" "--prefix" "$WEB_RUNTIME_WEB_DIR" "run" "dev" "--" "--hostname" "127.0.0.1" "--port" "$WEB_PORT"
}
