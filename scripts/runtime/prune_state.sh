...
find ".runtime-cache/temp" -mindepth 1 -mtime +2 -exec rm -rf {} + 2>/dev/null || true
...
find ".runtime-cache/temp" -type f \( -name '*.pyc' -o -name '*.pyo' \) -delete 2>/dev/null || true

echo "[prune-runtime-state] pruned runtime state using built-in TTL policy"
