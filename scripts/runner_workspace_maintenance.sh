...
usage() {
  cat <<'EOF'
Usage: bash scripts/runner_workspace_maintenance.sh --workspace <path> [--include-runner-diag 0|1] [--runner-diag-max-age-days <n>]
EOF
}
...
cleanup_glob "$WORKSPACE/.venv"
cleanup_glob "$WORKSPACE/mutants"
cleanup_glob "$WORKSPACE/apps/web/node_modules"
cleanup_glob "$WORKSPACE/apps/web/.next"
cleanup_glob "$WORKSPACE/apps/web/.next-e2e-*"
