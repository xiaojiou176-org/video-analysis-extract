...
def collect_e2e_labels(e2e_root: Path) -> set[str]:
    labels: set[str] = set()
...
def main() -> int:
    ...
    e2e_labels = collect_e2e_labels(args.e2e_root)
    total = len(app_labels)
    covered = sorted(label for label in app_labels if label in e2e_labels)
    uncovered = sorted(label for label in app_labels if label not in e2e_labels)
