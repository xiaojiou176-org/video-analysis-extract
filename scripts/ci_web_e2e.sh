...
start_worker() {
  ...
  for _ in $(seq 1 30); do
    ...
    if [[ -s "$ROOT_DIR/.runtime-cache/web-e2e-worker.log" ]]; then
      log "web-e2e worker process is up and producing logs"
      return 0
    fi
    sleep 1
  done
  fail "web-e2e worker failed readiness probe (no log output within 30s)"
}
