...
vd_log_init "tests" "$SCRIPT_NAME" "$ROOT_DIR/.runtime-cache/logs/tests/e2e-live-smoke.jsonl"
...
fail() {
  ...
  vd_log error e2e_live_smoke_error "$*"
  vd_log error e2e_live_smoke_failure_kind "failure_kind=${FAILURE_KIND} diagnostics_path=${DIAGNOSTICS_PATH}"
  exit 1
}
