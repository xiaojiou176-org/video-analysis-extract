...
vd_log_init "tests" "$SCRIPT_NAME" "$ROOT_DIR/.runtime-cache/logs/tests/smoke-llm-real-local.jsonl"
...
if [[ -n "${VD_API_KEY:-}" ]]; then
  ...
  request_headers+=(-H "Authorization: Bearer ${VD_API_KEY}")
  AUTH_MODE="x_api_key"
fi
...
log() {
  vd_log info smoke_llm_real_local "$*"
}

fail() {
  vd_log error smoke_llm_real_local_error "$*"
  exit 1
}
