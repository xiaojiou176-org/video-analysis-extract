...
vd_log_init "tests" "$SCRIPT_NAME" "$ROOT_DIR/.runtime-cache/logs/tests/api-real-smoke-local.jsonl"
...
log() {
  vd_log info api_real_smoke_local "$*"
}

fail_with_kind() {
  local kind="$1"
  local reason="$2"
  vd_log error api_real_smoke_local_error "failure_kind=${kind} reason=${reason}"
  exit 1
}
