...
function evaluateForm(form: HTMLFormElement): void {
  ...
  if (form.dataset.autoDisableRequired === "true") {
    ...
    disabled = requiredFields.some((field) => isBlank((field as HTMLInputElement).value) || !field.checkValidity());
  }
  ...
  dependentFields.forEach((field) => {
    ...
    const controller = form.querySelector<HTMLInputElement>(`input[type="checkbox"][name="${controllerName}"]`);
...
function evaluateAllForms(): void {
  ...
  document
    .querySelectorAll<Element>("form[data-auto-disable-required], form[data-require-one], form [data-disabled-unless-checked]")
    .forEach((node) => {
  ...
  forms.forEach((form) => evaluateForm(form));
}
