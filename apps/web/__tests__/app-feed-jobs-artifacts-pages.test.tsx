...
vi.mock("@/components/relative-time", () => ({
  RelativeTime: ({ dateTime }: { dateTime: string }) => <time data-testid="relative-time">{dateTime}</time>,
}));
...
describe("feed/jobs/artifacts pages", () => {
  ...
  it("renders feed list with filters and next page link", async () => {
    ...
    expect(screen.getByRole("link", { name: "查看产物" })).toHaveAttribute("href", "/artifacts?job_id=job-abcdef123");
  ...
  it("renders jobs page details and artifact links", async () => {
    mockGetJob.mockResolvedValue({
      ...
      steps: [],
      degradations: [{ step: "llm_digest", status: "degraded", reason: "timeout", error: null, error_kind: null, retry_meta: null, cache_meta: null }],
    ...
    expect(screen.getByRole("link", { name: "查看产物页" })).toHaveAttribute("href", "/artifacts?job_id=job-1");
  ...
  it("renders artifacts with embedded screenshots and markdown", async () => {
    ...
    render(await ArtifactsPage({ searchParams: { video_url: "https://www.youtube.com/watch?v=xyz" } }));
