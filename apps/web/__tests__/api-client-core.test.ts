...
describe("apiClient core behavior", () => {
  ...
  it("maps bad request to ERR_INVALID_INPUT", async () => {
    ...
    await expect(apiClient.sendNotificationTest({ subject: "x" })).rejects.toThrow("ERR_INVALID_INPUT");
  });
  ...
  it("sends JSON payload with no-store caching", async () => {
    const fetchSpy = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue(new Response(JSON.stringify({ enqueued: 0, candidates: [] }), { status: 200 }));
