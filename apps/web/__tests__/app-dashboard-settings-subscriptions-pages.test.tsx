...
describe("dashboard/settings/subscriptions pages", () => {
  ...
  it("renders dashboard metrics, table rows and failure CTA", async () => {
    ...
    expect(screen.getByRole("link", { name: "job-111" })).toHaveAttribute("href", "/jobs?job_id=job-111");
    expect(screen.getByRole("link", { name: "job-333" })).toHaveAttribute("href", "/jobs?job_id=job-333");
  });
  ...
  it("renders subscriptions page list and batch panel", async () => {
    ...
    render(await SubscriptionsPage({ searchParams: { status: "error", code: "ERR_INVALID_INPUT" } }));
  ...
  it("renders settings page config values and load failure message", async () => {
    ...
    render(await SettingsPage({ searchParams: { status: "success", code: "NOTIFICATION_CONFIG_SAVED" } }));
