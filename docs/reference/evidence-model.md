...
`check_run_manifest_completeness.py` 像“卷宗装订检查器”。它会确认：台账里写的日志文件真实存在、日志 metadata 的 `source_run_id` 真能对上台账的 `run_id`，并且当前这次运行的 logs/reports/evidence 确实都被 evidence index 收进去。这样 run manifest 才不是孤零零一张封面。
