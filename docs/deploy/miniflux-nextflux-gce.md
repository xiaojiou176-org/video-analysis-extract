...
Create `.env.reader-stack` in repo root:
...
```bash
./scripts/deploy_reader_stack.sh logs --env-file .env.reader-stack
./scripts/deploy_reader_stack.sh restart --env-file .env.reader-stack
./scripts/deploy_reader_stack.sh down --env-file .env.reader-stack
```
