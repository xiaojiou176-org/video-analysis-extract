...
- `web-test-build` 会追加阻断式 `Gemini UI/UX audit`，并上传 `.runtime-cache/reports/ui-audit/gemini-ui-ux-audit-*.{json,log}` 作为审查工件；严格通过条件为 `status=passed`、`reason_code=ok`、`successful_batches==batch_count` 且 `model_attempts>0`。
...
- `build-ci-standard-image.yml` 现在会在 hosted runner 上先显式准备 Docker Buildx，再调用 `scripts/ci/build_standard_image.sh` 执行多架构标准镜像构建；如果这条链在入口就失败，优先按 buildx/runtime-setup 问题排障，而不是直接归类为 GHCR 权限问题。
...
- PR 旁路 workflow（`pre-commit.yml`、`contract-diff.yml`、`env-governance.yml`、`vendor-governance.yml`）现在也必须先调用共享的 hosted `trusted-pr-boundary` reusable workflow，不能再绕过主 `ci.yml` 直接落到 self-hosted runner。
...
- 禁止安慰剂断言（`python3 scripts/governance/check_test_assertions.py --path .`）。
- E2E 严格性守卫必须通过（`python3 scripts/governance/check_e2e_strictness.py`，拦截硬等待与成功/失败混合断言模式）。
- 变异目标范围守卫必须通过（`python3 scripts/governance/check_mutation_scope.py`，防止 `paths_to_mutate` 缩水或丢失核心模块）。
- 变异测试选择守卫必须通过（`python3 scripts/governance/check_mutation_test_selection.py`，防止 `pytest_add_cli_args_test_selection` 缩水导致大量 `no_tests`）。
...
- Web 覆盖率硬门禁：`lines/functions/branches` 三指标均需满足 `global >=95%` 且 `core >=95%`（默认读取 `.runtime-cache/reports/web-coverage/coverage-summary.json`）。
- Web 交互覆盖门禁：`python3 scripts/governance/check_web_button_coverage.py --threshold 1.0 --e2e-threshold 0.6 --unit-threshold 0.93`，分别校验 combined / E2E / unit 三段口径。
...
- `pre-push` 与远端 CI `preflight-fast`/`web-test-build` 关键阻断项对齐：`contract surface gate`、`docs env canonical guard`、`provider residual guard`、`worker line limits guard`、`schema parity gate`、`web design token guard`、`web build`、`web button coverage`。
- Env 预算门禁强制执行（防反弹）：`core<=20`、`runtime<=100`、`scripts<=120`、`universe<=216`（`python3 scripts/governance/check_env_budget.py`）。
...
- 输入：`.runtime-cache/reports/web-coverage/coverage-summary.json`（Vitest `json-summary` 输出）。
...
- `pre-push`：
  ...
  - `python3 -c '...读取 .runtime-cache/reports/mutation/mutmut-cicd-stats.json 并校验 score/effective_ratio/no_tests_ratio 阈值...'`（CI/Hook 常用阈值 `score>=0.64`；脚本裸跑默认 `0.64`）
...
temporal server start-dev --ip 127.0.0.1 --port 7233 > .runtime-cache/logs/tests/web-e2e-temporal.log 2>&1 &
uv run --with uvicorn uvicorn apps.api.app.main:app --host 127.0.0.1 --port 18081 \
  > .runtime-cache/logs/tests/web-e2e-real-api.log 2>&1 &
```
...
- 禁止项：`actions/cache` 或工具缓存环境变量不得指向 `~/.cache/**`、`${{ github.workspace }}/**`、相对 repo 路径（如 `.runtime-cache/**`、`.cache/**`、`cache/**`、repo-side Python venv/browser cache 路径）。这些路径会在 shared self-hosted runner 上制造工作区污染。
