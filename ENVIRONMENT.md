...
- `GITHUB_SERVER_URL`, `GITHUB_REPOSITORY` are optional GitHub Actions metadata variables consumed by `scripts/ci/build_standard_image.sh` to stamp the OCI `org.opencontainers.image.source` label without hard-coding a fork-specific source URL.
