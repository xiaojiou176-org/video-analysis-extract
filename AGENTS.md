...
- 触发或重跑任意远程 CI 前，必须先本地执行并通过：`./scripts/quality_gate.sh --mode pre-push --heartbeat-seconds 20 --mutation-min-score 0.60 --profile ci --profile live-smoke --skip-mutation 1`。
